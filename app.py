...
def render_about() -> None:
    ...
    st.markdown(
        "Este prototipo combina lectura local del PDF con consultas públicas de metadatos. Crossref distribuye "
        "datos de retractaciones y actualizaciones, incluidos registros de Retraction Watch, a través de su API; "
        "las retractaciones aparecen en el campo `update-to`. [1] PubMed se consulta mediante las E-utilities "
        "de NCBI. [2] Ninguna fuente, alerta o ausencia de alerta equivale a un veredicto editorial."
    )
    st.markdown(
...
def main() -> None:
    ...
    st.caption("MVP para Doma · evidencias trazables, alertas explicables y decisión final humana.")
    ...
    with st.sidebar:
        ...
        if LLM_ENABLED:
            ...
            st.info("Síntesis asistida por API: no configurada. El extractor operará con citas literales locales.")
    ...
    tabs = st.tabs(["Detector", "Extractor", "Panel humano", "Historial", "Acerca del MVP"])
    with tabs[0]:
